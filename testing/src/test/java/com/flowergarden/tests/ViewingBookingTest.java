...
/**
 * ViewingBookingTest - Test viewing booking functionality
 * Flow: Register Account → Login → Search Hotel → Book Room → View Booking
 */
public class ViewingBookingTest extends TestBase {
    ...
    private String generateUniqueEmail() {
        ...
        return "viewingbookingtest" + timestamp + "@example.com";
    }
    
    @Test(description = "Test complete booking flow and viewing booking details")
    public void testCompleteBookingAndViewing() {
        helper = new PageObjectHelper(driver);
        
        try {
            System.out.println("🚀 Starting complete booking and viewing test...");
            ...
            System.out.println("❌ Booking and viewing test failed: " + e.getMessage());
            helper.mediumDelay();
            Assert.fail("Booking and viewing test failed: " + e.getMessage());
        }
    }
    ...
    private void searchForHotels() {
        ...
        try {
            ...
            // Method 4: Try clicking submit button directly with JavaScript
            if (!searchSubmitted) {
                try {
                    System.out.println("  🔄 Trying direct JavaScript button click...");
                    ((org.openqa.selenium.JavascriptExecutor) driver)
                        .executeScript("document.querySelector('button[type=\"submit\"], input[type=\"submit\"]').click();");
                    searchSubmitted = true;
                    System.out.println("  ✅ Search submitted using direct JavaScript click");
                } catch (Exception e) {
                    System.out.println("  ⚠️ Direct JavaScript click failed: " + e.getMessage());
                }
            }
            
            // Method 5: Try pressing Enter key on the form
            if (!searchSubmitted) {
                try {
                    System.out.println("  🔄 Trying Enter key submission...");
                    ((org.openqa.selenium.JavascriptExecutor) driver)
                        .executeScript("document.querySelector('form').dispatchEvent(new KeyboardEvent('keydown', {key: 'Enter', keyCode: 13}));");
                    searchSubmitted = true;
                    System.out.println("  ✅ Search submitted using Enter key");
                } catch (Exception e) {
                    System.out.println("  ⚠️ Enter key submission failed: " + e.getMessage());
                }
            }
        ...
        try {
            ...
            if (helper.isElementDisplayed(MY_RESERVATIONS_LINK)) {
                ...
                By[] altReservationLinks = {
                    ...
                    By.xpath("//a[contains(text(), 'Bookings')]"),
                ...
                for (By altLink : altReservationLinks) {
                    if (helper.isElementDisplayed(altLink)) {
                        System.out.println("  🔄 Trying alternative reservation link: " + altLink);
                        try {
                            helper.clickElement(altLink);
                            reservationsLinkFound = true;
                            System.out.println("  ✅ Alternative reservation link clicked");
                            break;
                        } catch (Exception e) {
                            System.out.println("  ⚠️ Alternative link click failed: " + e.getMessage());
                        }
                    }
                }
            }
            ...
            boolean onReservationsPage = currentUrl.contains("reservation") || 
                                       currentUrl.contains("booking") ||
                                       currentUrl.contains("my") ||
                                       helper.isElementDisplayed(RESERVATION_LIST);
            
            if (!onReservationsPage) {
                System.out.println("  ❌ Not on reservations page");
    ...
    /**
     * Display reservation details from the reservations page
     */
    private void displayReservationDetails() {
        try {
            // Look for reservation list
            if (helper.isElementDisplayed(RESERVATION_LIST)) {
                System.out.println("  📋 Found reservation list:");
                var reservations = helper.getElements(RESERVATION_LIST);
                System.out.println("    Total reservations found: " + reservations.size());
                
                for (int i = 0; i < Math.min(reservations.size(), 3); i++) {
                    try {
                        String reservationText = reservations.get(i).getText();
                        if (!reservationText.trim().isEmpty()) {
                            System.out.println("    Reservation " + (i + 1) + ": " + reservationText.substring(0, Math.min(150, reservationText.length())) + "...");
                        }
                    } catch (Exception e) {
                        System.out.println("    Reservation " + (i + 1) + ": [Could not read reservation details]");
                    }
                }
            }
            
            // Look for specific reservation details
            if (helper.isElementDisplayed(RESERVATION_DETAILS)) {
                System.out.println("  📄 Found reservation details:");
                var details = helper.getElements(RESERVATION_DETAILS);
                System.out.println("    Total detail sections found: " + details.size());
                
                for (int i = 0; i < Math.min(details.size(), 2); i++) {
                    try {
                        String detailText = details.get(i).getText();
                        if (!detailText.trim().isEmpty()) {
                            System.out.println("    Detail " + (i + 1) + ": " + detailText.substring(0, Math.min(200, detailText.length())) + "...");
                        }
                    } catch (Exception e) {
                        System.out.println("    Detail " + (i + 1) + ": [Could not read detail]");
                    }
                }
            }
            
            // Look for booking status information
            By statusElements = By.xpath("//span[contains(@class, 'status')] | //div[contains(@class, 'status')] | //*[contains(text(), 'Confirmed')] | //*[contains(text(), 'Pending')] | //*[contains(text(), 'Active')]");
            if (helper.isElementDisplayed(statusElements)) {
                var statuses = helper.getElements(statusElements);
                System.out.println("  📊 Found " + statuses.size() + " status elements");
                for (int i = 0; i < Math.min(statuses.size(), 3); i++) {
                    try {
                        String statusText = statuses.get(i).getText();
                        if (!statusText.trim().isEmpty()) {
                            System.out.println("    Status " + (i + 1) + ": " + statusText);
                        }
                    } catch (Exception e) {
                        System.out.println("    Status " + (i + 1) + ": [Could not read status]");
                    }
                }
            }
            
            // Look for booking dates
            By dateElements = By.xpath("//*[contains(text(), '2025')] | //*[contains(text(), '11/11')] | //*[contains(text(), '13/11')] | //*[contains(@class, 'date')]");
            if (helper.isElementDisplayed(dateElements)) {
                var dates = helper.getElements(dateElements);
                System.out.println("  📅 Found " + dates.size() + " date elements");
                for (int i = 0; i < Math.min(dates.size(), 5); i++) {
                    try {
                        String dateText = dates.get(i).getText();
                        if (!dateText.trim().isEmpty()) {
                            System.out.println("    Date " + (i + 1) + ": " + dateText);
                        }
                    } catch (Exception e) {
                        System.out.println("    Date " + (i + 1) + ": [Could not read date]");
                    }
                }
            }
            
            System.out.println("  ✅ Reservation details displayed successfully");
            
        } catch (Exception e) {
            System.out.println("  ⚠️ Could not display reservation details: " + e.getMessage());
        }
    }
}
